...
export function Chat({
    ...
    return (
        ...
        >
            ...
            />
            {/* Header */}
            <header className="h-12 px-4 flex items-center justify-between border-b border-(--color-border)">
                ...
                <div className="flex items-center gap-1">
                    ...
                    <button
                        onClick={onClose}
                        className="p-1 text-(--color-text-muted) hover:text-(--color-text) hover:bg-(--color-bg-muted) rounded-lg"
                    >
                        <svg
                            width="16"
                            height="16"
                ...
                (isLoadingSuggestions ||
                    ...
                    <div
                        className={`px-4 pb-2 pt-4 flex flex-wrap gap-2 transition-opacity duration-150 border-t border-(--color-border) ${
                            hasBottomOverflow
                                ? "shadow-[0_-8px_16px_-8px_rgba(0,0,0,0.1)] dark:shadow-[0_-8px_16px_-8px_rgba(0,0,0,0.3)]"
                                : ""
                        }`}
                        style={{
                            opacity: input ? 0 : 1,
                        ...
                        {isLoadingSuggestions ? (
                            ...
                            followUpSuggestions?.map((suggestion) => (
                                <button
                                    ...
                                    className="text-left text-xs px-3 py-2 border border-(--color-border) rounded-lg text-(--color-text-muted) hover:border-(--color-text-muted) hover:text-(--color-text)"
                                >
                                    {suggestion}
                                </button>
                            ))
                ...
                >
                    {[
                        ...
                        <button
                            ...
                            className="block text-left text-xs px-3 py-2 border border-(--color-border) rounded-lg text-(--color-text-muted) hover:border-(--color-text-muted) hover:text-(--color-text)"
                        >
                            {suggestion}
                        </button>
                    ))}
            ...
            >
                <textarea
                    ...
                    placeholder={
                        isDemo || hasApiKey ? "Ask anything..." : "Need API key"
                    }
